...
export default async function DashboardPage() {
  ...
  const trendConfig: ChartConfig = {
    [clusterNames[0] ?? "cluster"]: { label: clusterNames[0] ?? "Cluster", color: "hsl(var(--chart-1))" },
    [clusterNames[1] ?? ""]: { label: clusterNames[1] ?? "", color: "hsl(var(--chart-2))" },
    [clusterNames[2] ?? ""]: { label: clusterNames[2] ?? "", color: "hsl(var(--chart-3))" },
  };
